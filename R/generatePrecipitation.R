...
#' fit_monthly   <- fitdistrForPrecipitation(data=prec_mes,dname="gamma",start=NULL,sample="monthly",origin=origin)
#' 
#' ## Estimate coefficients for Precipitation Occurence Modeling  (using generate.YuleWalkerCoefficientBlockmatrices)
...
#' 
#' generation_monthly_2 <- generate(CCGamma_monthly,year_min=year_min,year_max=year_max,names=names(prec_mes),precipitation.indicator=TRUE)
#'  
...
#' abline(0,1)
#' 
#' CCGamma_monthly_gen <- CCGamma(data=prec_gen,lag=0,tolerance=0.001,only.matrix=FALSE,sample="monthly",origin=origin)
#' CCGamma_monthly_gen_2 <- CCGamma(data=prec_gen_2,lag=0,tolerance=0.001,only.matrix=FALSE,sample="monthly",origin=origin)
#' 
#' 
